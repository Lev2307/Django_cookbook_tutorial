...
def try_to_include(parser, token):
    ...
    try:
        ...
        raise template.TemplateSyntaxError(f"{tag_name} tag requires a single argument")
    return IncludeNode(template_name)


# load Querysets with params
...
# parse a value as a template and render it immediately or store it in a context variable 
# working with urls
...
# tags, which let you add, change, or remove the parameters of the current query.

def construct_query_string(context, query_params):
    # empty values will be removed
    query_string = context["request"].path
    if len(query_params):
        encoded_params = urlencode([(key, force_str(value)) for (key, value) in query_params if value]).replace("&", "&amp;")
